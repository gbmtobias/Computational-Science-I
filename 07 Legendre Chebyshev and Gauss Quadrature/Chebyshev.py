...
for i in range(1, N):
    ...
    for j in range(i):
        ...
        f = lambda x: evalp(polynomial[i], x) * evalp(polynomial[j], x) / sqrt(1 - x**2)
        
        integral1 = fivepoint(f, -1, 1, B)
        
        f = lambda x: evalp(polynomial[j], x) * evalp(polynomial[j] , x) / sqrt(1 - x**2)
...
for i in range(2, N):          
    # Normalization
    f = lambda x: evalp(polynomial[i], cos(x)) * evalp(polynomial[i], cos(x))
